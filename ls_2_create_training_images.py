...
class_0_folder = 'ls_training_images/class_0_no_ls'
...
clear_images(class_1_folder)
clear_images(class_0_folder)
...
def process_intervals_and_save_images(data, grid_boxes, class_1_chance=.5, class_0_chance=.05):
    threshold = 2 # Number of images away from the boundary between sp and no sp
                ...
                for i in range(space, num_frames - space):
                    for interval in intervals:
                        if interval[0] + threshold <= i <= interval[1] - threshold:
                            if random.random() < class_1_chance:
                                save_image(radiance, class_1_folder, orbit_number, i, box, grid_boxes)
                        elif all(i < interval[0] - threshold or i > interval[1] + threshold for interval in intervals):
                            if random.random() < class_0_chance:
                                save_image(radiance, class_0_folder, orbit_number, i, box, grid_boxes)
