...
def update_plot(time_step):
    ...
    # Calculate the running average for mlls over a x-frame window
    kernel = np.ones(running_average_window) / running_average_window
    averaged_mlls = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode='same'), axis=0, arr=mlls)
    averaged_mlls_at_time = averaged_mlls[current_time_step, :, :]
    ...
    # Highlight boxes with averaged MLLS > threshold and count them
    cloud_count = 0
    ...
    if show_highlight:
        for box, coords in grid_boxes.items():
            ...
            # Use the averaged MLLS to determine highlights
            if averaged_mlls_at_time[y_idx, x_idx] > threshold:
                cloud_count += 1
