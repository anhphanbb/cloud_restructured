...
def process_intervals_and_save_images(data, grid_boxes, cloud_chance=0.45, no_cloud_chance=0.018):
    threshold = 3  # Number of images away from the boundary between sp and no sp
    orbit_intervals = extract_intervals_per_orbit(data)

    # Define special rules for certain orbits and intervals
    special_rules = {
        1: {'double_cloud': [(944, 1171)], 'triple_no_cloud': [(0, 500)]},
        45: {'double_cloud': [(560, 1000)], 'triple_no_cloud': []}
    }
        ...
        with Dataset(nc_file_path, 'r') as nc:
            ...
            for box, intervals in boxes.items():
                ...
                for i in range(space, num_frames - space):
                    adjusted_cloud_chance = cloud_chance
                    adjusted_no_cloud_chance = no_cloud_chance

                    # Apply special rules for the current orbit
                    if orbit_number in special_rules:
                        for interval in special_rules[orbit_number].get('double_cloud', []):
                            if interval[0] <= i <= interval[1]:
                                adjusted_cloud_chance *= 2

                        for interval in special_rules[orbit_number].get('triple_no_cloud', []):
                            if interval[0] <= i <= interval[1]:
                                adjusted_no_cloud_chance *= 3

                    for interval in intervals:
                        if interval[0] + threshold <= i <= interval[1] - threshold:
                            if random.random() < adjusted_cloud_chance:
                            ...
                            if random.random() < adjusted_no_cloud_chance:
